...
1. Start a new R project, and create a new Rmarkdown file (.Rmd), knitting to HTML

- Update YAML (there are a lot of options that can be included here...fow now, we'll just update the title and our name)
...
'biofuels.csv': Global biofuels consumption & production data (thousand barrels per day): https://catalog.data.gov/dataset/biofuels-consumption-and-production-by-country-2000-2010
...
5. Gather to get this in long format using tidyr::gather (note: or use pivot_longer from the development version of tidyr using devtools::install_github("hadley/tidyr") - they do the same thing, but the pivot_longer syntax is more intuitive than gather). 
...
# Use head(biofuels_long) in the console to see the first 5 lines, ensuring that it's been gathered (or View() to see the whole df). Also notice that the class of 'year' is still a character (since it's been converted from a column name, which is always a character). Let's change the class of the year column to numeric (dbl can integer or numeric). 

biofuels_gather$year <- as.numeric(biofuels_gather$year)

# Now check the class again (you can use class(biofuels_long$year) in the console)
...
wb_gather$year <- as.numeric(wb_gather$year)
...
*There are other options for joins, but full_join is kind of the safest (all data is retained, even if there are matches in both datasets).*
...
df_subset <- df_join %>% 
  ...
  rename(tot_ghg = indicator_value) %>% 
...
Let's say we want to calculate the total greenhouse gas emissions (kt CO2 equivalents) and biofuels consumption (thousands barrels per day) for each country from 2000 - 2010. We'll use group_by + summarize:
...
totals <- df_subset %>% 
  ...
  summarize(
    ghg = sum(tot_ghg),
    biofuel = sum(biofuel_amount)*365*10*1000
  )
...
13. Let's make a graph of some sort...

```{r, echo = FALSE, message = FALSE, fig.width = 5, fig.height = 4}

ggplot(us_df, aes(x = year, y = biofuel_amount)) +
  geom_point() +
  geom_line() +
